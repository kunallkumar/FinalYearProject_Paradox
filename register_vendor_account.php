...
<head>
 ...
 if(isset($_POST['v_register'])){
   ...
   $query = mysql_query("INSERT INTO vendor (v_f_name,v_phn,v_email,v_add,v_pin,v_pass,v_img,v_L_name,v_age,v_gen,v_city,v_state) value('$v_f_name','$v_phn','$v_email','".mysql_real_escape_string($v_add)."','$v_pin','$v_pass','$v_img','$v_L_name','$v_age','$v_gen','$v_city','$v_state')");
   $v_code_update=mysql_query("UPDATE vendor SET v_code = concat( vstr, v_id )");
   send_sms($_POST['v_phn'],"Dear " . $_POST['v_f_name'] . ", your account for paradox is created successfully :) KEEP IN TOUCH");
   $v_code=mysql_query("SELECT v_code FROM vendor WHERE v_email='$v_email'");
   $mail = new PHPMailer(true);
     ...
     try{
      $mail->isSMTP();
       ...
       $mail->SMTPAuth = true;
       $mail->Username = 'kk74987@gmail.com'; // Gmail address which you want to use as SMTP server
       $mail->Password = '1234divya*'; // Gmail address Password
       $mail->SMTPSecure = "tls";
       $mail->Port = 587;
       $mail->CharSet = "UTF-8";

       $mail->setFrom('kk74987@gmail.com'); // Gmail address which you used as SMTP server
